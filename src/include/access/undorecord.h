...
typedef struct UndoCompressionInfo
{
	...
	FullTransactionId fxid;		/* full transaction id */
} UndoCompressionInfo;
...
#define UREC_INFO_PAGE_COMMON  (UREC_INFO_RMID | UREC_INFO_RELOID | UREC_INFO_FXID)
 ...
 */
typedef struct UndoRecordLogSwitchs
{
...
typedef enum UndoUnPackStage
{
	UNDO_UNPACK_STAGE_UNDO_INFO,	/* We don't yet know the undo record
									 * length so the next field to be unpacked
									 * is undo info to know the optional
									 * headers length. */
	UNDO_UNPACK_STAGE_HEADERS,	/* The next thing to unpack is all the headers
								 * except the payload header */
	UNDO_UNPACK_STAGE_PAYLOAD_DATA, /* The next thing to be unpacked is the
									 * payload data. */
	UNDO_UNPACK_STAGE_TUPLE_DATA,	/* The next thing to be unpacked is the
									 * tuple data. */
	UNDO_UNPACK_STAGE_DONE		/* complete */
}			UndoUnPackStage;
...
/* Size of the main header of the undo record. */
#define SizeOfUndoRecordHeader \
		(offsetof(UnpackedUndoRecord, uur_cid) + sizeof(CommandId))
...
{
	UndoUnPackStage stage;		/* Undo unpack stage */
	int			already_processed;	/* Number of bytes read/written so far */
	int			partial_bytes;	/* Number of partial bytes read/written */
	uint16		undo_len;			/* During insert it stores undo record
									 * length whereas during unpack it stores
									 * optional headers length. */
	UndoRecordPayload urec_payload; /* Payload header */
	char	   *pack_data;		/* Packed undo record data. */
	UnpackedUndoRecord *uur;	/* Unpacked undo record. */
} UndoPackContext;

extern size_t UndoRecordExpectedSize(UnpackedUndoRecord *uur);
extern size_t UndoRecordPayloadSize(UnpackedUndoRecord *uur);
extern char *SerializeUndoRecord(UnpackedUndoRecord *uur, size_t size);
extern void BeginInsertUndo(UndoPackContext *ucontext, char *pack_data,
							uint16 undo_len);
extern bool InsertUndoData(UndoPackContext *ucontext, Page page,
						   int offste, bool skip);
extern void BeginUnpackUndo(UndoPackContext *ucontext, UnpackedUndoRecord *uur);
