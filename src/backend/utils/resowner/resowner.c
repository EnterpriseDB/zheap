							 ...
							 bool isTopLevel)
{
	...
	{
		/*
		 * For aborts, we don't want to release the locks immediately if we
		 * have some pending undo actions to perform.  Instead, we release
		 * them after applying undo actions.  See ApplyUndoActions.
		 */
		if (isTopLevel)
		{
			/*
