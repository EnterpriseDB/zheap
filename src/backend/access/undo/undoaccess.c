...
struct PreparedUndoSpace
{
	...
	char	   *pack_undo;		/* packed undo data. */
						...
						int size, int undo_offset)
{
	...
	/*
	 * Remember the undo record pointer, offset of the bytes in page and
	 * number of bytes we want to update.
	 */
	urec_update_info->urecptr = urecptr;
	urec_update_info->offset = starting_byte;
	urec_update_info->size = size;
 ...
 *
 * urecptr - undo record pointer to be updated.
 */
static void
							...
							UndoRecPtr urecptr)
{
	...
	PrepareUndoRecordUpdate(context, urecptr, sizeof(UndoRecPtr), offset);
 ...
 */
static void
UndoRecordUpdate(UndoRecordInsertContext *context, int idx, char *sourceptr)
{
	Page		page = NULL;
	int			i = 0;
	int			write_offset;
	...
	while (1)
	{
		...
		can_write = Min((BLCKSZ - write_offset), size);
				  ...
				  Oid dbid)
{
	 ...
	 */
	if (UndoRecPtrIsValid(prevlog_xact_start))
		PrepareUndoRecordUpdateNext(context, prevlog_xact_start);
	 ...
	 */
	if (need_xact_header)
		PrepareUndoRecordUpdateNext(context, last_xact_start);
	...
	/*
	 * Serialize the unpacked undo record, and store in the prepared undo
	 * space.
	 */
	prepared_undo->pack_undo = SerializeUndoRecord(urec, size);
				 ...
				 PreparedUndoSpace * prepared_undo)
{
	...
	BeginInsertUndo(&ucontext, prepared_undo->pack_undo, prepared_undo->size);
	...
	while (1)
	{
		...
		/*
		 * Insert the record to the current block if the buffer is valid.
		 * Otherwise, just skip writing it because during recovery, there
		 * might be some blocks which are already deleted due to some discard
		 * command. We still need to call InsertUndoData (with skip flag set)
		 * so that the offset is updated correctly for inserting the remaining
		 * record to the next block incase the record is split and the next
		 * block is valid.
		 */
		if (BufferIsValid(buffer))
		{
			 ...
			 */
			res = InsertUndoData(&ucontext, page, starting_byte, false);
			MarkBufferDirty(buffer);
		}
		else
		{
			Assert(InRecovery);
			res = InsertUndoData(&ucontext, NULL, starting_byte, true);
		}
...
InsertPreparedUndo(UndoRecordInsertContext *context)
{
	...
	for (i = 0; i < context->nurec_update_info; i++)
		UndoRecordUpdate(context, i, (char *) &prepared_undo->urp);
}
...
FinishUndoRecordInsert(UndoRecordInsertContext *context)
{
	...
	/*
	 * Release the memory for packed data and the internal header which we
	 * have allocated during the prepare time.
	 */
	for (i = 0; i < context->nprepared_undo; i++)
	{
		pfree(context->prepared_undo[i].pack_undo);
 ...
 */
static void
UndoPageGetCompressionInfo(UnpackedUndoRecord *uur, UndoRecPtr urp, Page page)
{
	int			offset;
	...
	if ((uur->uur_info & UREC_INFO_RMID) == 0)
		uur->uur_rmid = compresssion_info.rmid;

	if ((uur->uur_info & UREC_INFO_RELOID) == 0)
		uur->uur_reloid = compresssion_info.reloid;

	if ((uur->uur_info & UREC_INFO_FXID) == 0)
		uur->uur_fxid = compresssion_info.fxid;
}
				 ...
				 UndoLogCategory category, Buffer *curbuf)
{
	...
	while (true)
	{
		 ...
		 */
		if (ucontext.stage == UNDO_UNPACK_STAGE_DONE)
		{
			/*
	 ...
	 * If any of these common field is missing from the undo record then it's
	 * guarantee that the first complete record on the page must be from the
	 * same transaction otherwise we will always include all these common
	 * fields in the undo record.
	 */
	if ((urec->uur_info & UREC_INFO_PAGE_COMMON) != UREC_INFO_PAGE_COMMON)
	{
		LockBuffer(*curbuf, BUFFER_LOCK_SHARE);
		UndoPageGetCompressionInfo(urec, urp, BufferGetPage(*curbuf));
 ...
 *
 * Read undo records between end_urecptr and to_urecptr until we exhaust the
 * the memory size specified by max_result_size.  This will start reading undo
 * records starting from end_urecptr.  It will read the transaction's undo
 ...
 * previous log as the end_urecptr so that the caller can call this API again
 ...
 *
 * end_urecptr		- This is the end location of the last undo record from
 *					  where we need to start fetching.  If we can not read all
 *					  the records because of memory limit then this will be set
 *					  to the point upto which we have already fetched so that we
 *					  resume from the point we we left.
 * to_urecptr		- While fetching the undo record backward we need to fetch
 *					  upto this undo record pointer.
 * max_result_size	- Memory segment limit to collect undo records.
 * nrecords			- Number of undo records read.
 */
UndoRecInfo *
UndoBulkFetchRecord(UndoRecPtr *end_urecptr, UndoRecPtr to_urecptr,
					int max_result_size, int *nrecords)
{
	 ...
	 * always detect that?
	 */
	if (UndoRecPtrGetLogNo(*end_urecptr) == UndoRecPtrGetLogNo(to_urecptr))
	...
	logno = UndoRecPtrGetLogNo(*end_urecptr);
	slot = UndoLogGetSlot(logno, true);
	if (slot == NULL)
	{
		if (BufferIsValid(buffer))
	...
	/*
	 * The caller has provided us the end pointer that is the end of the last
	 * undo record.  From that we can compute the actual undo record pointer of
	 * that record so that we can read that record.
	 */
	urecptr = UndoGetPrevUrp(*end_urecptr, InvalidBuffer, category);
	UndoRecPtrAssignRelFileNode(rnode, urecptr);

	*end_urecptr = InvalidUndoRecPtr;
	...
	while (1)
	{
		...
		urp_array[urp_index].urp = urecptr;
		...
		/*
		 * If we have fetched all the undo record we wanted then stop processing
		 * further.
		 */
		if (urecptr == to_urecptr)
		{
			*end_urecptr = InvalidUndoRecPtr;
			break;
		}
		/*
		 * If the record contain the log switch header that means this is the
		 * first record for the transaction in this undo log after the log
		 * switch happened.  So the previous undo record pointer to be fetched
		 * will be in previous log.  We can get the previous logno from the log
		 * switch header and from there we can compute the end undo record
		 * pointer in that log.
		 */
		else if (uur->uur_logswitch)
		{
			*end_urecptr = MakeUndoRecPtr(logno, UndoLogGetNextInsertPtr(
		 ...
		 * to set the end_urecptr so that on next call we can resume fetching
		 * undo records where we left it.
		 */
		else if (total_size >= max_result_size)
		{
			*end_urecptr = urecptr;
			break;
		}
		else
			urecptr = UndoGetPrevUrp(prev_urec_ptr, buffer, category);
	}
 ...
 */
UndoRecPtr
UndoGetPrevUrp(UndoRecPtr urp, Buffer buffer,
			   UndoLogCategory category)
{
