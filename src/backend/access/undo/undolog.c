	...
	*last_xact_start =
	...
	if (*need_xact_header)
	{
		if (UndoRecPtrIsValid(*prevlog_xact_start))
			SetUndoLogXactStart(context->category, *prevlog_xact_start);
		else if (UndoRecPtrIsValid(GetUndoLogXactStart(context->category)))
			*prevlog_xact_start = GetUndoLogXactStart(context->category);
	}
