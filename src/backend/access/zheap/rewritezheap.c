...
static void raw_zheap_insert(RewriteState state, ZHeapTuple tup);
					...
					MultiXactId cutoff_multi, bool use_wal)
{
	...
	/* We don't hash tables for zheap as we copy on LIVE tuples. */
	state->rs_unresolved_tups = NULL;
	state->rs_old_new_tid_map = NULL;
 ...
 */
void
end_zheap_rewrite(RewriteState state)
{
						  ...
						  bool newRelHasOids, RewriteState rwstate)
{
	...
	/* Preserve OID, if any */
	if (newRelHasOids)
		ZHeapTupleSetOid(copiedTuple, HeapTupleGetOid(tuple));
 ...
 */
void
rewrite_zheap_tuple(RewriteState state, ZHeapTuple old_tuple,
 ...
 */
static void
raw_zheap_insert(RewriteState state, ZHeapTuple tup)
{
