					...
					CommandId *cid)
{
	...
	if (vis_info & ITEMID_XACT_INVALID)
	{
		...
		FetchTransInfoFromUndo(&undo_tup, InvalidTransactionId, &zinfo, false);
	}
	else
		zinfo.cid =
					   ...
					   ZHeapTupleTransInfo *zinfo)
{
	...
	if (is_invalid_slot)
	{
		...
		FetchTransInfoFromUndo(zhtup, InvalidTransactionId, zinfo, true);
	}
...
fetch_prior_undo_record:
	...
	if (zinfo.trans_slot != prev_trans_slot_id)
	{
		GetTransactionSlotInfo(buffer,
							   ...
							   true,
							   &zinfo);
		FetchTransInfoFromUndo(undo_tup, zinfo.xid, &zinfo, false);
				 ...
				 ItemPointer ctid, int trans_slot)
{
	...
	while (1)
	{
		...
		Assert(zhtup == NULL ||
			   ItemPointerGetOffsetNumber(&zhtup->t_self) == offnum);
		...
		zinfo.epoch_xid = FullTransactionIdFromEpochAndXid(epoch,
														   urec->uur_prevxid);
 ...
 *
 *	We traverse the undo chain to get the visible tuple if any, in case
