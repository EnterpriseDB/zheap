	...
	if (fctx->call_cntr < fctx->max_calls)
	{
			...
			lp_offset + lp_len <= raw_page_size)
		{
				...
				ztuphdr->t_hoff == MAXALIGN(ztuphdr->t_hoff))
			{
				...
				if (ztuphdr->t_infomask & ZHEAP_HASOID)
					values[9] = ZHeapTupleHeaderGetOid(ztuphdr);
				else
					nulls[9] = true;
			}
			else
			...
			memcpy(VARDATA(tuple_data_bytea), (char *) ztuphdr + ztuphdr->t_hoff,
				   tuple_data_len);
			values[10] = PointerGetDatum(tuple_data_bytea);

			decode_infomask(ztuphdr, &values[11], &nulls[11]);
		}
		else
		{
			/*
